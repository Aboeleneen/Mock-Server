...
export const generateTransactionResponse = async (request: TransactionRequest) => {
    ...
    transactions = transactions.filter((transaction: Transaction) => {
        ...
        if (merchantId && merchantId.length)  includeItem = includeItem && merchantId.includes(transaction.merchantId); // transaction.merchantId = faker.helpers.arrayElement(merchantId)
