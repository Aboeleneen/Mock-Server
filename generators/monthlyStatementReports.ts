...
export const getMonthlyStatementReports = async (request: MonthlyStatementReportRequest) => {
    ...
    monthlyStatementReports = monthlyStatementReports.filter((report: MonthlyStatementReport) => {
        ...
        if (request.storeIds && request.storeIds.length) report.storeId = faker.helpers.arrayElement(request.storeIds)// includeItem = includeItem && request.storeIds.includes(report.storeId);
        return includeItem;
    }
    )
