...
export const getTaxInvoices = async (request: TaxInvoicesRequest): Promise<TaxInvoicesResponse> => {
    ...
    taxInvoicesData = taxInvoicesData.filter((invoice: TaxInvoice) => {
        ...
        if (request.merchantId && request.merchantId.length) invoice.merchantId = faker.helpers.arrayElement(request.merchantId) // includeItem = includeItem && request.storeIds.includes(invoice.storeId);
        return includeItem;
    }
    )
    return {
